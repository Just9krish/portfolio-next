...
import { ProjectItem, PROJECTS_DATA } from '@/utilities/data';
...
function ProjectCard({ project }: { project: ProjectItem }) {
  return (
    <motion.div
      className="group relative overflow-hidden rounded-xl border bg-background shadow-md transition-all h-full flex flex-col"
      whileHover={{ y: -5 }}
    >
      ...
      {/* Project Content */}
      <div className="p-6 flex flex-col gap-10 justify-between flex-1 relative z-10 min-h-0">
        <div className="space-y-4">
          <div className="flex items-start justify-between">
            <Link href={`/projects/${project.slug}`}>
              <h3 className="text-xl font-bold hover:underline transition-all">
                {project.title}
              </h3>
            </Link>
            <motion.div
